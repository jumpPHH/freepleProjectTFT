    ...
    function fn_checkEmail() {
        // 1. 입력한 Email의 정보.. ( JQuery 활용 변경 )
        const mb_email = {"mb_email": $("#mb_email").val()};

        // 2. XMLHttpRequest 객체 활용 X -> ( JQuery 활용 변경 )
        $.ajax({
            type: 'POST',                        // HTTP 요청 방식(GET, POST)
            url: '../login/loginFindIdProcess',  // 클라이언트가 요청을 보낼 서버의 URL 주소
            data: JSON.stringify(mb_email),    // HTTP 요청과 함께 서버로 보낼 데이터
            contentType: "application/json; charset=UTF-8", // JSON 타입으로 UTF-8로 받겠다.
            dataType : "json",                  // 데이터의 타입은 JSON 이다.
            success : function(data) {

                const checkEmailAlertBox = $("#checkEmailAlert");
                ...
                if(data){
                    ...
                    checkEmailAlertBox.css("border", "2px solid #787878");
                    checkEmailAlertBox.text("입력을 하지 않으셨거나 등록된 이메일이 아닙니다.\n소셜 회원이라면 가입한 이메일을 입력 해주세요.");
                    checkEmailAlertBox.removeClass("text-success");
                    checkEmailAlertBox.addClass("text-danger");
                }
            }
